        ...
        <div class="relative max-h-[92vh] w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
            ...
            <div class="max-h-[72vh] overflow-y-auto px-6 py-5">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <label><span class="text-slate-500 uppercase" style="font-size:11px;font-weight:600">Semester</span><select data-field="semester" class="mt-1.5 h-10 w-full rounded-md border border-slate-200 px-3"><option>Semester 1</option><option>Semester 2</option><option>Semester 3</option></select></label>
                    <label><span class="text-slate-500 uppercase" style="font-size:11px;font-weight:600">Registration opens</span><input data-field="registrationStart" type="date" class="mt-1.5 h-10 w-full rounded-md border border-slate-200 px-3" /></label>
                    <label><span class="text-slate-500 uppercase" style="font-size:11px;font-weight:600">Registration closes</span><input data-field="registrationEnd" type="date" class="mt-1.5 h-10 w-full rounded-md border border-slate-200 px-3" /></label>
                    ...
                    <label><span class="text-slate-500 uppercase" style="font-size:11px;font-weight:600">Add/drop closes</span><input data-field="addDropEnd" type="date" class="mt-1.5 h-10 w-full rounded-md border border-slate-200 px-3" /></label>
...
<asp:Content ContentPlaceHolderID="ScriptsPlaceholder" runat="server">
    ...
    (function(){
      ...
      function loadSession(id){var e=events.find(function(x){return x.SessionId===id});if(!e)return;set("sessionId",e.SessionId);set("intakeId",e.IntakeId);set("intakeMonth",iso(parse(e.IntakeMonth)).slice(0,7));set("semester",e.SemesterName);set("registrationStart",iso(parse(e.RegistrationStart)));set("registrationEnd",iso(parse(e.RegistrationEnd)));set("startDate",iso(parse(e.SessionStartDate)));set("addDropEnd",iso(parse(e.AddDropEnd)));set("endDate",iso(parse(e.SessionEndDate)));set("status",e.Status==="Completed"?"Completed":"Scheduled")}
      document.addEventListener("click",function(ev){var p=ev.target.closest("[data-calendar-prev]"),n=ev.target.closest("[data-calendar-next]"),today=ev.target.closest("[data-calendar-today]"),edit=ev.target.closest("[data-edit-session],[data-calendar-edit]"),del=ev.target.closest("[data-calendar-delete]");if(p){cursor.setMonth(cursor.getMonth()-1);render()}else if(n){cursor.setMonth(cursor.getMonth()+1);render()}else if(today){cursor=new Date(new Date().getFullYear(),new Date().getMonth(),1);render()}else if(edit){loadSession(edit.dataset.editSession||(edit.closest("tr")&&edit.closest("tr").dataset.sessionId))}else if(del){ev.preventDefault();ev.stopImmediatePropagation();if(confirm("Delete this intake semester and its lifecycle?"))fetch("academic_calendar.aspx/DeleteAcademicSession",{method:"POST",headers:{"Content-Type":"application/json; charset=utf-8"},body:JSON.stringify({sessionId:del.dataset.sessionId})}).then(function(r){if(!r.ok)throw 0;location.reload()})}else if(ev.target.closest("[data-new-semester]")){["sessionId","intakeId","intakeMonth","registrationStart","registrationEnd","startDate","addDropEnd","endDate"].forEach(function(x){set(x,"")});set("semester","Semester 1");set("status","Scheduled")}else if(ev.target.closest("[data-semester-save]")){var month=field("intakeMonth").value,name=month?new Date(month+"-01T00:00:00").toLocaleDateString("en-US",{month:"long",year:"numeric"}).toUpperCase():"";fetch("academic_calendar.aspx/SaveAcademicSession",{method:"POST",headers:{"Content-Type":"application/json; charset=utf-8"},body:JSON.stringify({request:{sessionId:field("sessionId").value,intakeId:field("intakeId").value,intakeName:name,intakeMonth:month+"-01",semester:field("semester").value,registrationStart:field("registrationStart").value,registrationEnd:field("registrationEnd").value,startDate:field("startDate").value,addDropEnd:field("addDropEnd").value,endDate:field("endDate").value,status:field("status").value}})}).then(function(r){if(!r.ok)throw 0;location.reload()}).catch(function(){if(window.toast)window.toast.error("Check the lifecycle dates and try again.")})}},true);
