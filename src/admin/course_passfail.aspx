          ...
          html += '<section class="rounded-lg border border-slate-200 bg-white">' +
                    '<div class="flex flex-col gap-5 px-6 py-6 lg:flex-row lg:items-start lg:justify-between">' +
                      ...
                      '<div class="flex items-center gap-2">' +
                        '<button type="button" data-toast="Pass/Fail CSV exported" data-toast-desc="' + esc(c.code + ' · ' + c.title) + '" class="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 h-10 text-slate-700 hover:bg-slate-50 transition-colors" style="font-size:13px;font-weight:600"><i data-lucide="download" class="h-4 w-4"></i> Export CSV</button>' +
                      '</div>' +
                    '</div>' +
                  '</section>';
